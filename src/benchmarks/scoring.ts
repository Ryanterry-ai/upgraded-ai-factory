...
export const WEIGHTS: ScoringCriteria = {
  buildSuccess: 0.20,
  typecheck: 0.10,
  lint: 0.05,
  seo: 0.10,
...
export function calculateTotalScore(scores: ScoringCriteria): number {
  return (
    scores.buildSuccess * WEIGHTS.buildSuccess +
...
export function calculateGrade(totalScore: number): ScoredOutput['grade'] {
  if (totalScore >= 0.97) return 'A+';
  if (totalScore >= 0.93) return 'A';
  if (totalScore >= 0.90) return 'A-';
  if (totalScore >= 0.87) return 'B+';
  if (totalScore >= 0.83) return 'B';
  if (totalScore >= 0.80) return 'B-';
  if (totalScore >= 0.77) return 'C+';
  if (totalScore >= 0.73) return 'C';
  if (totalScore >= 0.70) return 'C-';
  if (totalScore >= 0.60) return 'D';
  return 'F';
}

export function calculatePassRate(scores: ScoringCriteria): number {
  const passing = [
...
export function identifyStrengths(scores: ScoringCriteria): string[] {
  const strengths: string[] = [];

  if (scores.buildSuccess >= 0.9) strengths.push('Excellent build reliability');
  if (scores.typecheck >= 0.9) strengths.push('Strong type safety');
  if (scores.lint >= 0.9) strengths.push('Clean code quality');
  if (scores.seo >= 0.9) strengths.push('Great SEO optimization');
  if (scores.accessibility >= 0.9) strengths.push('Excellent accessibility');
  if (scores.performance >= 0.9) strengths.push('Strong performance');
  if (scores.mobileUx >= 0.9) strengths.push('Great mobile experience');
  if (scores.featureCompleteness >= 0.9) strengths.push('Complete feature set');
  if (scores.designQuality >= 0.9) strengths.push('High design quality');
  if (scores.promptAccuracy >= 0.9) strengths.push('Accurate prompt following');

  return strengths;
}

export function identifyIssues(scores: ScoringCriteria): string[] {
  const issues: string[] = [];

  if (scores.buildSuccess < 0.7) issues.push('Build failures detected');
  if (scores.typecheck < 0.7) issues.push('TypeScript errors present');
  if (scores.lint < 0.7) issues.push('Lint errors detected');
  if (scores.seo < 0.6) issues.push('Missing SEO essentials');
  if (scores.accessibility < 0.6) issues.push('Accessibility issues');
  if (scores.performance < 0.6) issues.push('Performance problems');
  if (scores.mobileUx < 0.6) issues.push('Poor mobile experience');
  if (scores.featureCompleteness < 0.6) issues.push('Missing key features');
  if (scores.designQuality < 0.6) issues.push('Design quality issues');
  if (scores.promptAccuracy < 0.6) issues.push('Poor prompt following');

  return issues;
}

export function createScoredOutput(
  promptId: string,
  platform: string,
  scores: ScoringCriteria
): ScoredOutput {
...
export function compareOutputs(outputs: ScoredOutput[]): ComparisonResult {
  if (outputs.length < 2) {
...
export function generateReport(results: ComparisonResult[]): BenchmarkReport {
  const platforms = [...new Set(results.flatMap(r => r.platforms.map(p => p.platform)))];

  const overallScores: Record<string, PlatformScore> = {};
  ...
  const categoryScores: Record<string, Record<string, PlatformScore>> = {};
